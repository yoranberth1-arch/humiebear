...
function updatePricingCards() {
  ...
  if (candyTitle) candyTitle.textContent = '€10 p.p. • 500 gram snoep • 2 uur';
  ...
  if (comboTitle) comboTitle.textContent = '€29,50 p.p. • 2 uur';
  ...
  const waffleCard = cards.find((card) => card.querySelector('.price-label')?.textContent.includes('WAFFLE')) || cards[1];
  if (waffleCard) {
    const waffleTitle = waffleCard.querySelector('h3');
    const waffleParagraph = waffleCard.querySelector('p');
    const waffleList = waffleCard.querySelector('ul');
    if (waffleTitle) waffleTitle.textContent = '€20 p.p. • 2 uur all-in';
    if (waffleParagraph) waffleParagraph.textContent = 'Verse lollywafels, milkshakes, ijs, koffie en warme chocomelk inbegrepen voor 2 uur.';
    if (waffleList) {
      waffleList.innerHTML = `
        <li>Vanaf 20 personen</li>
        <li>€20 per persoon</li>
        <li>2 uur all-in</li>
        <li>Koffie inbegrepen</li>
